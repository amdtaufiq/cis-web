<?php
    ...
    $queryPrestasi = mysqli_query($koneksi,"
        SELECT catatan_prestasi.*, user.* FROM `catatan_prestasi`
        LEFT JOIN user ON catatan_prestasi.user_id=user.user_id
    ...
    while($row=mysqli_fetch_assoc($querySiswa)){                   
        ...
        $pdf->Cell(0,3,'REKAP PRESTASI SISWA',0,1,'C');
        $pdf->Cell(0,5,'',0,1);
        ...
        $pdf->Cell(85,6,'Prestasi',1,1,'C');
        
        $pdf->SetFont('Arial','',10);
        
        if (mysqli_num_rows($queryPrestasi) > 0){
            $no=1;
            while($roww=mysqli_fetch_array($queryPrestasi)){
                $cellWidth=85; 
        	    $cellHeight=6; 
        	
            	if($pdf->GetStringWidth($roww['nama_prestasi']) < $cellWidth){
            		$line=1;
            	}else{
            		
            		$textLength=strlen($roww['nama_prestasi']);	
            		$errMargin=6;		
            		$startChar=0;		
            		$maxChar=0;			
            		$textArray=array();	
            		...
            		while($startChar < $textLength){ 
            			...
            			($startChar+$maxChar) < $textLength ) {
            				$maxChar++;
            				$tmpString=substr($roww['nama_prestasi'],$startChar,$maxChar);
            			}
            ...
            $pdf->Cell(35,($line * $cellHeight),date("d F Y", strtotime($roww['tanggal_prestasi'])),1,0,'L');
            $pdf->Cell(35,($line * $cellHeight),$roww['nama_user'],1,0,'L');
            $pdf->MultiCell($cellWidth,$cellHeight,$roww['nama_prestasi'],1,1,'L');
        
            }
        }else{
            $pdf->Cell(165,6,'Tidak terdapat catatan prestasi',1,1,C);
        }
