...
if(isset($_POST['import'])){
	...
	foreach($sheet as $row){
		...
		if($numrow > 1){
			...
			$query_id = mysqli_query($koneksi,"SELECT mapel_id FROM mapel WHERE kode_mapel='$kode_mapel'");
			$result = mysqli_fetch_array($query_id);
			$mapel_id = $result['mapel_id'];

			$query = "INSERT INTO `user`(`nama_user`,`nip`, `mapel_id`, `username`, `password`, `level`) 
