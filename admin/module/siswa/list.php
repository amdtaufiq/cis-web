    ...
    <div>
        <form method="POST">  
            <div class="col-md-auto">
                ...
                <a  class='btn mb-1 btn-danger btn-md float-right' id="btnResetPoin" href='#' style="margin-right: 5px;">Reset Poin</a>
            ...
            <tbody>
                <?php
                    ...
                    $query = "SELECT siswa.*,kelas.*, jurusan.* 
                                ...
                                WHERE kelas.tingkat_kelas LIKE ? AND kelas.tipe_kelas LIKE ? AND jurusan.jurusan_id LIKE ? ORDER BY siswa.nis ASC";
                    ...
                    if($res->num_rows > 0){
                        while ($row = $res->fetch_assoc()){
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    echo "<input id='".$siswa_id."' style='margin-right:10px;' onclick='onChangeTindakan(".$siswa_id.")' type='checkbox'"; if($row['status_siswa'] == 1 ){echo 'checked';} echo ">";
                                    if($row['status_siswa']==1){echo "Aktif";}else{echo "Non-aktif";}
                                    ?>
                                </td>
                ...
                <form id="formTambah">
                    <div class="modal-body">  
                        ...
                        <div class="form-group col-lg-12">
                            ...
                            <input type="number" class="form-control" id="poin_pelanggaran_siswa" name="poin_pelanggaran_siswa" autocomplete="off" autocomplete="off" onkeypress="return event.charCode >= 48 && event.charCode <= 57" value=0 min="0" max="100">
                        </div>
        ...
        <div class="modal-content">
            ...
            <form id="formUbah">
                <div class="modal-body">
                    ...
                    <div class="form-group col-lg-12">
                        <label>Poin Pelanggaran Siswa <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="poin_pelanggaran_siswa1" name="poin_pelanggaran_siswa1" autocomplete="off" onkeypress="return event.charCode >= 48 && event.charCode <= 57" min="0" max="100">
                    </div>                    
...
<script type="text/javascript">
    ...
    function onChangeTindakan(siswa_id) {
        ...
        xhr.open('POST', './module/siswa/update-status.php', true);
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send(`siswa_id=${siswa_id}`);
        ...
        if ($('#poin_pelanggaran_siswa').val()==""){
            ...
            var data = $('#formTambah').serialize();
                $.ajax({
                    ...
                    console.log(result);                                
                        if (result==="sama") {
                            swal("Gagal!", "Kode siswa sudah terdaftar", "error");
            ...
            swal("Peringatan!", "Nomor Induk Siswa (NIS) tidak boleh kosong", "warning");
                    ...
                    $.ajax({
                        ...
                        success: function(result){ 
                            console.log(result);                                 
                        if (result==="sama") {
                            swal("Gagal!", "NIS siswa sudah terdaftar", "error");
                        } else if (result==="sukses") {
            ...
            swal({
                ...
                function () {
                    $.ajax({
                        ...
                        success: function(result){                          
                            if (result==="sukses") {
                                swal("Sukses!", "NIS siswa berhasil dihapus.", "success");
                                setTimeout(() => window.location.reload(), 1000);
                            } else {
    ...
    document.getElementById('btnResetPoin').addEventListener("click", function(){
        swal({
            title: "Apakah Anda Yakin?",
            text: "Akan mereset poin siswa \n Menyebabkan semua data catatan pelanggaran akan hilang",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
        ...
        function () {
            $.ajax({
                type : "POST",
                url  : "./module/siswa/reset-poin.php",
                data : {},
                success: function(result){                          
                    if (result==="sukses") {
                        swal("Sukses!", "Data poin siswa berhasil direset.", "success");
                        setTimeout(() => window.location.reload(), 1000);
                    } else {
                        swal("Gagal!", "Data poin siswa tidak bisa direset.", "error");
                        setTimeout(() => window.location.reload(), 1000);
                    }
                }
            });
